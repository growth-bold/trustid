...
import {
  IconSearch, IconArrowRight, IconArrowLeft, IconStarFilled, IconCheck, IconLock,
  IconInfoCircle, IconShieldCheck, IconWritingSign, IconDownload, IconShield,
} from "@tabler/icons-react";
import { Seal } from "@/com/brand/Seal";
import { DuotoneIcon } from "@/com/icon/duotone";
import { cn } from "@/lib/cn";
import {
  subjects as SUBJECTS, ELEM_LABEL, USAGE_BASE, USAGE_LABEL, TERR_MULT, TERR_LABEL,
  usageOptions, operatorTypes, filterDefs, operatorNames, expiryByTerm, postLicenseDuties,
} from "@/data/marketplace";

const OK = "#00bc7d";

const Switch = ({ on }) => (
  <span className={cn("relative h-[25px] w-11 shrink-0 rounded-full transition-colors", on ? "bg-[#00bc7d]" : "bg-[#cfd6e0]")}>
    <span className={cn("absolute top-[2.5px] h-5 w-5 rounded-full bg-white shadow-[0_1px_3px_rgba(0,0,0,.28)] transition-[left]", on ? "left-[21px]" : "left-[2.5px]")} />
  </span>
);

const RadioDot = ({ on }) => (
  <span className={cn("box-border h-5 w-5 shrink-0 rounded-full bg-white transition", on ? "border-[6px] border-blue" : "border-2 border-[#cfd6e0]")} />
);
...
export const LicenseApp = () => {
  ...
  const [f, setF] = useState({ subject: "all", usage: "all", territory: "all", budget: "all", element: "all" });
  const [selected, setSelected] = useState("ma");
  const [cstep, setCstep] = useState(1);
  const [opType, setOpType] = useState("dn");
  const [usage, setUsage] = useState({ ads: true, ai: false, dub: false, campaign: false });
  const [elements, setElements] = useState({ portrait: true, voice: false, style: false, name: false });
  const [cfg, setCfg] = useState({ channel: "social", territory: "vn", term: 12, propose: 20 });
  const [priceModel, setPriceModel] = useState("fixed");
  const [comp, setComp] = useState({ provenance: false, scope: false, dlcn: false, takedown: false });
  ...
  const elemCount = ["portrait", "voice", "style", "name"].filter((k) => elements[k]).length;
  ...
  const usageSummary = usageKeys.length ? usageKeys.map((k) => USAGE_LABEL[k]).join(", ") : "Chưa chọn";
  const enabledElems = ["portrait", "voice", "style", "name"].filter((k) => elements[k]);
  const elementSummary = enabledElems.length ? enabledElems.map((k) => ELEM_LABEL[k]).join(", ") : "Chưa chọn";
  const termLabel = `${cfg.term} tháng`;
  const scopeSummary = `${TERR_LABEL[cfg.territory] || ""} · ${termLabel}`;
  const priceModelLabel =
    priceModel === "fixed" ? "Giá cố định" : priceModel === "share" ? "Chia sẻ doanh thu 15%" : `Đề xuất ${cfg.propose} tr ₫`;
  ...
  if (priceModel === "share") {
    ...
    totalSubcaption = total > 0 ? `Trọn gói ${termLabel} · đã gồm phí bảo chứng` : "Chọn loại hình để tính chi phí";
    totalLabel = "Tổng tạm tính";
  }

  const costLines = [
    { label: `Loại hình (${usageKeys.length})`, value: usageBase > 0 ? `${usageBase} tr ₫` : "—" },
    { label: `Yếu tố nhân dạng (${elemCount})`, value: `×${elemMult.toFixed(2)}` },
    { label: `Lãnh thổ ${TERR_LABEL[cfg.territory] || ""}`, value: `×${terrMult.toFixed(1)}` },
  ...
  const contractBlocks = [
    { n: "1", title: "Các bên", value: `${operatorTypes.find((o) => o.key === opType).title} ↔ ${d.name}` },
    ...
    { n: "5", title: "Điều cấm", value: `${d.prohibited.length} điều khoản khoá` },
  ...
  const shared = {
    d, usageSummary, elementSummary, scopeSummary, priceModelLabel,
    totalValue, totalSubcaption, totalLabel, costLines, contractBlocks,
    operatorName: opName, licenseCode, expiryDate,
  };
  ...
  return (
    <div className="min-h-screen bg-ivory font-sans">
      {view === "market" && (
        <Marketplace
          tab={tab} setTab={setTab} q={q} setQ={setQ} f={f} setF={setF}
      ...
      {view === "detail" && (
        <SubjectDetail
          d={d}
          onBack={() => { setView("market"); setTab("market"); }}
          onConfig={() => { setView("config"); setCstep(1); }}
        />
      )}
      {view === "config" && (
        <ConfigWizard
          {...shared}
          cstep={cstep} setCstep={setCstep}
          opType={opType} setOpType={setOpType}
          usage={usage} toggleUsage={toggleUsage}
          elements={elements} toggleElem={toggleElem}
          cfg={cfg} setCfg={setCfg}
          priceModel={priceModel} setPriceModel={setPriceModel}
      ...
      {view === "issued" && (
        <LicenseIssued {...shared} onBackToMarket={() => { setView("market"); setTab("market"); }} />
      )}
...
const Marketplace = ({ tab, setTab, q, setQ, f, setF, onOpen }) => {
  const list = SUBJECTS.filter((s) => {
    ...
    if (qv && !`${s.name} ${s.typeLabel}`.toLowerCase().includes(qv)) return false;
    return true;
  });
  ...
  return (
    <div data-reveal>
      ...
      >
        <div aria-hidden className="pointer-events-none absolute right-[-70px] top-2.5 opacity-[.06]">
        ...
        <div className="relative mx-auto max-w-[1200px] px-8 pb-[30px] pt-[46px]">
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-[640px]">
              ...
              <h1 className="mt-[18px] text-balance font-serif text-[38px] font-bold leading-[1.16] text-white">
                Tìm chủ thể, cấu hình phạm vi, ký &amp; nhận giấy phép minh bạch.
              </h1>
              <p className="mt-3.5 text-base leading-[1.65] text-[#c3cede]">
                Dành cho doanh nghiệp, agency và nền tảng cần khai thác hình ảnh, giọng nói hay thương
                hiệu cá nhân một cách hợp pháp — mọi giấy phép đều có bằng chứng bất biến.
              </p>
            </div>
            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  tab === key ? "bg-white text-navy shadow-[0_2px_8px_rgba(0,0,0,.15)]" : "text-[#c3cede]"
                )}
              >
                {label}
              </button>
          ...
          {tab === "market" && (
            <>
              {/* Search */}
              <div data-reveal className="mt-3.5 flex flex-wrap items-center gap-2.5 rounded-2xl bg-white p-3.5 shadow-[0_24px_54px_rgba(0,0,0,.32)]">
                <div className="flex min-w-[260px] flex-1 items-center gap-2.5 px-3.5">
                  <IconSearch size={20} stroke={1.6} className="shrink-0 text-mute" />
                  <input
                    value={q}
                ...
                <button className="shrink-0 rounded-xl bg-blue px-7 py-3.5 text-[15px] font-bold text-white shadow-[0_6px_16px_rgba(12,140,233,.3)]">
                  Tìm chủ thể
                </button>
              </div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-2.5 min-[720px]:grid-cols-2 min-[1080px]:grid-cols-5">
                {filterDefs.map((def) => (
                  <label key={def.key} className="flex flex-col gap-1.5">
                    <span className="text-[11px] font-semibold tracking-[0.02em] text-[#9fb0c8]">{def.label}</span>
                    <select
                      value={f[def.key]}
                      onChange={(e) => setF((s) => ({ ...s, [def.key]: e.target.value }))}
              ...
              <div className="mt-3.5 flex items-center gap-2.5 rounded-[11px] border border-white/[.12] bg-white/[.05] p-[10px_15px] text-[12.5px] text-[#9fb0c8]">
                <IconShieldCheck size={16} stroke={1.5} className="shrink-0 text-gold-2" />
                Mọi giấy phép được Hiệp hội bảo chứng, có{" "}
                <b className="font-semibold text-[#c3cede]">bằng chứng bất biến</b> (tem thời gian + mã
                băm) và tra cứu công khai bất cứ lúc nào.
              </div>
            </>
          )}
        </div>
        ...
        <main data-reveal className="mx-auto max-w-[760px] px-8 pb-[60px] pt-9">
          <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-[0_6px_18px_rgba(15,23,43,.05)]">
            <div className="px-7 pb-1.5 pt-6">
              <div className="text-xs font-bold uppercase tracking-[0.16em] text-gold">Yêu cầu trực tiếp</div>
              ...
              <p className="mt-2 text-sm leading-[1.6] text-mute">
                Đã biết chủ thể cần khai thác? Nhập mã bảo hộ hoặc tên để gửi yêu cầu cấp phép trực
                tiếp. Chủ thể sẽ nhận, duyệt và phản hồi qua kênh bảo chứng.
              </p>
            </div>
            <div className="flex flex-col gap-4 px-7 pb-[26px] pt-[18px]">
              <div>
                ...
                <input placeholder="Ví dụ: TID-2026-0087 · Nguyễn M. A." className={inputCls} />
              </div>
              <div>
                <label className="mb-1.5 block text-[13px] font-semibold text-ink">Mô tả nhu cầu khai thác</label>
                <textarea rows={4} placeholder="Loại hình, phạm vi, thời hạn, ngân sách dự kiến…" className={cn(inputCls, "resize-y")} />
              </div>
              <div className="flex items-start gap-3 rounded-[11px] border border-[#cfe6fb] bg-[#eef7ff] p-[13px_15px] text-[13px] leading-[1.55] text-[#2a6699]">
                <IconInfoCircle size={17} stroke={1.6} className="mt-px shrink-0 text-blue" />
                Gửi yêu cầu <b>không</b> tạo nghĩa vụ tài chính. Chi phí chỉ hình thành khi hai bên ký
                hợp đồng điện tử.
              </div>
              <button
      ...
      {tab === "market" && (
        <main className="mx-auto max-w-[1200px] px-8 pb-16 pt-8">
          <div className="mb-[18px] flex flex-wrap items-baseline justify-between gap-4">
            <h2 className="font-serif text-[22px] font-bold text-navy">Chủ thể đang nhận cấp phép</h2>
          ...
          {list.length > 0 ? (
            ...
            <div className="rounded-2xl border border-dashed border-[#cfd6e0] bg-white p-11 text-center">
              ...
              <div className="mt-3.5 text-[17px] font-bold text-navy">Không có chủ thể nào khớp bộ lọc</div>
              <div className="mx-auto mt-1.5 max-w-[440px] text-sm leading-[1.55] text-mute">
                Thử nới lỏng loại chủ thể, ngân sách hoặc yếu tố nhân dạng — hoặc gửi yêu cầu trực tiếp
                tới chủ thể bạn đã biết.
              </div>
              <div className="mt-[18px] flex justify-center gap-2.5">
                <button
                  onClick={() => setF({ subject: "all", usage: "all", territory: "all", budget: "all", element: "all" })}
  ...
  <div className="flex flex-col rounded-2xl border border-line bg-white p-5 shadow-[0_6px_18px_rgba(15,23,43,.05)] transition hover:-translate-y-[3px] hover:border-[#cfe0f2] hover:shadow-[0_14px_34px_rgba(15,23,43,.12)]">
    <div className="flex items-center gap-[13px]">
      <span
        ...
        style={{ color: s.tint, background: `${s.tint}1f`, border: `1px solid ${s.tint}44` }}
      >
        {s.initials}
      </span>
      <div className="min-w-0 flex-1">
        <div className="text-base font-bold leading-[1.25] text-navy">{s.name}</div>
        <div className="mt-0.5 text-[12.5px] text-mute">{s.typeLabel}</div>
      </div>
      <span
        className={cn(
          "inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold",
          s.accepting ? "bg-[#d6f7ea] text-[#007a55]" : "bg-[#fff3dd] text-[#8a5000]"
        )}
      >
    ...
    <div className="mt-2 flex flex-wrap gap-1.5">
      {s.elements.map((k) => (
        <span key={k} className="inline-flex items-center gap-1.5 rounded-full border border-line bg-ivory px-2.5 py-[5px] text-xs font-medium text-navy">
          {ELEM_LABEL[k]}
        </span>
      ))}
    </div>
    <div className="mt-4 flex items-end justify-between gap-2.5 border-t border-line pt-3.5">
      <div>
        ...
        <div className="mt-px font-serif text-[18px] font-bold text-navy">{s.priceText}</div>
      </div>
...
const KickerLabel = ({ children }) => (
  <div className="text-xs font-bold uppercase tracking-[0.14em] text-gold">{children}</div>
);

const SubjectDetail = ({ d, onBack, onConfig }) => (
  <main data-reveal className="mx-auto max-w-[1080px] px-8 pb-16 pt-[26px]">
    <button onClick={onBack} className="inline-flex items-center gap-2 py-2 text-sm font-semibold text-mute">
    ...
    <div className="mt-3 grid grid-cols-1 items-center gap-[22px] rounded-[18px] border border-line bg-white p-[26px_28px] shadow-[0_6px_18px_rgba(15,23,43,.05)] min-[720px]:grid-cols-[auto_1fr_auto]">
      <span
        ...
        style={{ color: d.tint, background: `${d.tint}1f`, border: `1px solid ${d.tint}44` }}
      >
        {d.initials}
      </span>
      <div>
        <div className="flex flex-wrap items-center gap-2.5">
          <h1 className="font-serif text-[28px] font-bold text-navy">{d.name}</h1>
        ...
        <div className="mt-1 text-[14.5px] text-mute">
          {d.typeLabel} · Mã bảo hộ <b className="font-mono text-navy">{d.code}</b>
        </div>
        <div className="mt-2.5 inline-flex items-center gap-2 text-[13.5px] text-navy">
          ...
          <span className="font-medium text-mute">· {d.reviewsCount} đánh giá uy tín</span>
        </div>
      </div>
      <div className="text-right">
        <div className="text-[11px] uppercase tracking-[0.04em] text-[#8a93a3]">Chính sách giá</div>
        <div className="mt-0.5 font-serif text-[22px] font-bold text-navy">{d.priceText}</div>
    ...
    <div className="mt-6 grid grid-cols-1 items-start gap-6 min-[1080px]:grid-cols-[1fr_340px]">
      <div className="flex flex-col gap-[18px]">
        <div className="rounded-2xl border border-line bg-white p-[22px_24px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
          ...
          <p className="mt-2.5 text-[14.5px] leading-[1.65] text-ink">{d.bio}</p>

          <div className="mt-5 text-[13px] font-semibold text-ink">Yếu tố cho phép cấp phép</div>
          <div className="mt-2.5 flex flex-wrap gap-2">
            {d.elements.map((k) => (
              <span key={k} className="inline-flex items-center gap-1.5 rounded-full border border-[#cfe6fb] bg-[#eef7ff] px-3 py-[7px] text-[13px] font-semibold text-[#0b5c9c]">
                <IconCheck size={13} stroke={1.9} />
                {ELEM_LABEL[k]}
              </span>
            ))}
          ...
          <div className="mt-5 text-[13px] font-semibold text-ink">Điều cấm — không bao giờ cấp phép cho</div>
          <div className="mt-2.5 flex flex-wrap gap-2">
            {d.prohibited.map((p) => (
              <span key={p} className="inline-flex items-center gap-1.5 rounded-full border border-[#ffccce] bg-[#fff2f2] px-3 py-[7px] text-[13px] font-semibold text-[#9f0712]">
        ...
        <div className="rounded-2xl border border-line bg-white p-[22px_24px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
          ...
          <div className="mt-3.5 grid grid-cols-2 gap-x-6 gap-y-4">
            {[
              ...
              <div key={k}>
                <div className="text-[11px] font-semibold uppercase tracking-[0.04em] text-[#8a93a3]">{k}</div>
                <div className="mt-[3px] text-[14.5px] font-medium text-navy">{v}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-line bg-white p-[22px_24px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
          ...
          <div className="mt-3.5 flex flex-col gap-3.5">
            {d.reviews.map((r, i) => (
              <div key={i} className="rounded-xl border border-line bg-ivory p-[14px_16px]">
                <div className="flex items-center justify-between gap-2.5">
                  <div className="text-[13.5px] font-semibold text-navy">{r.author}</div>
                ...
                <p className="mt-2 text-[13.5px] leading-[1.55] text-ink">{r.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <aside className="sticky top-24 flex flex-col gap-4">
        ...
        >
          <div className="flex items-center gap-[11px] border-b border-white/[.12] pb-3.5">
            <Seal size={42} />
            <div>
              <div className="text-[15px] font-bold text-white">Cấp phép được bảo chứng</div>
              <div className="mt-px text-xs text-[#8ea0ba]">Hợp đồng điện tử · bằng chứng bất biến</div>
            </div>
          </div>
          <div className="mt-3.5 text-[13.5px] leading-[1.6] text-[#c3cede]">
            Cấu hình phạm vi khai thác theo đúng ranh giới chủ thể cho phép. Chi phí hiển thị realtime
            trước khi bạn ký.
          </div>
          <button
            onClick={onConfig}
        ...
        <div className="rounded-2xl border border-line bg-white p-[18px_20px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
          <div className="flex items-start gap-2.5">
            <IconCheck size={17} stroke={1.8} style={{ color: OK }} className="mt-px shrink-0" />
            <span className="text-[13px] leading-[1.5] text-ink">
              Chủ thể có thể <b className="font-semibold">rút uỷ quyền tức thì</b>; hệ thống hỗ trợ gỡ
              nhanh khi tranh chấp.
            </span>
          </div>
          <div className="mt-2.5 flex items-start gap-2.5">
            <IconCheck size={17} stroke={1.8} style={{ color: OK }} className="mt-px shrink-0" />
            <span className="text-[13px] leading-[1.5] text-ink">
              Mọi nội dung khai thác được gắn nhãn <b className="font-semibold">provenance</b>.
            </span>
          </div>
        </div>
      </aside>
...
const cStepLabels = ["Bên khai thác", "Phạm vi", "Báo giá", "Tuân thủ", "Ký & thanh toán"];
const cTitles = { 1: "Thông tin bên khai thác", 2: "Chọn phạm vi khai thác", 3: "Cơ chế tài chính & báo giá", 4: "Nghĩa vụ tuân thủ", 5: "Ký hợp đồng & thanh toán" };
const cDescs = {
...
const ConfigWizard = (props) => {
  const {
    d, cstep, setCstep, opType, setOpType, usage, toggleUsage, elements, toggleElem,
    cfg, setCfg, priceModel, setPriceModel, comp, setComp, signed, setSigned, payMethod, setPayMethod,
    onBack, onIssue, costLines, totalLabel, totalValue, totalSubcaption,
    usageSummary, elementSummary, scopeSummary, priceModelLabel, contractBlocks,
  } = props;
        ...
        <div>
                    ...
                    <div className="flex flex-col items-center gap-[7px]">
                      <span
                        className={cn(
                          ...
                          status === "active" && "border-2 border-blue bg-white text-blue shadow-[0_0_0_4px_rgba(12,140,233,.14)]",
                          status === "todo" && "border-[1.5px] border-line bg-ivory text-[#9aa6b6]"
                        )}
                      >
                        {n}
                      </span>
                      <span className={cn("whitespace-nowrap text-xs max-[720px]:hidden", status === "todo" ? "font-medium text-[#9aa6b6]" : "font-semibold text-navy")}>
                        {label}
                      </span>
                    </div>
                    {n < 5 && <span className={cn("h-0.5 min-w-[12px] flex-1 rounded-sm", cstep > n ? "bg-blue" : "bg-line")} />}
                  </div>
                );
              })}
            </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-[0_6px_18px_rgba(15,23,43,.05)]">
            <div className="px-7 pb-2 pt-6">
              ...
              <h2 className="mt-1.5 font-serif text-[24px] font-bold leading-[1.2] text-navy">{cTitles[cstep]}</h2>
              <p className="mt-2 text-[14.5px] leading-[1.6] text-mute">{cDescs[cstep]}</p>
            </div>

            <div className="px-7 pb-1 pt-[18px]">
              {cstep === 1 && <ConfigStep1 opType={opType} setOpType={setOpType} />}
              {cstep === 2 && (
                <ConfigStep2
                  usage={usage} toggleUsage={toggleUsage} elements={elements} toggleElem={toggleElem}
                  cfg={cfg} setCfg={setCfg} d={d} termLabel={termLabel}
                />
              )}
              {cstep === 3 && (
                <ConfigStep3
                  priceModel={priceModel} setPriceModel={setPriceModel} cfg={cfg} setCfg={setCfg}
                  costLines={costLines} totalLabel={totalLabel} totalValue={totalValue}
                />
              )}
              {cstep === 4 && <ConfigStep4 comp={comp} setComp={setComp} />}
              {cstep === 5 && (
                <ConfigStep5
                  contractBlocks={contractBlocks} signed={signed} setSigned={setSigned}
                  payMethod={payMethod} setPayMethod={setPayMethod}
                />
              )}
            </div>
              ...
              {cstep < 5 ? (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    signed ? "cursor-pointer bg-navy text-white shadow-[0_6px_16px_rgba(25,40,61,.3)]" : "cursor-not-allowed bg-[#e7ebf1] text-[#a2adbd]"
                  )}
                >
        ...
        <aside className="sticky top-24 flex flex-col gap-4">
          ...
          >
            ...
            <div className="relative flex items-center justify-between">
              <span className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.08em] text-gold-2">
                <DuotoneIcon name="text-file" size={16} color="#e0b64a" secondaryColor="#ecd9a3" />
                Tóm tắt giấy phép
              </span>
              <span className="text-[11px] text-[#8ea0ba]">Bước {cstep}/5</span>
            </div>
            <div className="mt-3.5 font-serif text-[19px] font-bold text-white">{d.name}</div>
            <div className="mt-0.5 text-[12.5px] text-[#9fb0c8]">{d.typeLabel}</div>
          ...
          <div className="rounded-2xl border border-line bg-white p-[18px_20px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
            <div className="flex items-center justify-between">
              <span className="text-[14.5px] font-bold text-navy">Chi phí tạm tính</span>
              <span className="inline-flex items-center gap-1.5 text-[11px] text-blue">
                <span className="h-[7px] w-[7px] rounded-full bg-blue" />
                Realtime
              </span>
            </div>
            <div className="mt-2 font-serif text-[30px] font-bold text-navy">{totalValue}</div>
            <div className="mt-0.5 text-[12.5px] text-mute">{totalSubcaption}</div>
            <div className="mt-3.5 h-1.5 overflow-hidden rounded-full bg-ivory">
              <span
                className="block h-full rounded-full transition-[width]"
                style={{ width: `${(cstep / 5) * 100}%`, background: "linear-gradient(90deg,var(--tid-blue),#0b6fbb)" }}
              />
            </div>
            <div className="mt-2 text-[11.5px] text-mute">Đã hoàn thành {cstep}/5 bước cấu hình</div>
          </div>
...
const ConfigStep1 = ({ opType, setOpType }) => (
  <div data-reveal className="flex flex-col gap-4">
    <div>
      <div className="mb-2.5 text-[13px] font-semibold text-ink">Loại bên khai thác</div>
        ...
        {operatorTypes.map((o) => (
          <button
            ...
            className={cn(
              ...
              opType === o.key ? "border-blue bg-[#f2f8ff]" : "border-line bg-white"
            )}
          >
    ...
    <div>
      <label className="mb-1.5 block text-[13px] font-semibold text-ink">Tên pháp nhân</label>
      <input placeholder="Ví dụ: Công ty TNHH AEON Việt Nam" className={inputCls} />
    </div>
    <div className="grid grid-cols-1 gap-3 min-[560px]:grid-cols-2">
      <div>
        <label className="mb-1.5 block text-[13px] font-semibold text-ink">Mã số thuế (MST)</label>
        <input placeholder="0100000000" className={inputCls} />
      </div>
      <div>
        <label className="mb-1.5 block text-[13px] font-semibold text-ink">Người đại diện</label>
        <input placeholder="Họ tên người ký" className={inputCls} />
      </div>
    </div>
    <div>
      <label className="mb-1.5 block text-[13px] font-semibold text-ink">Mục đích khai thác</label>
      <textarea rows={3} placeholder="Mô tả ngắn gọn chiến dịch / sản phẩm sẽ sử dụng nhân dạng…" className={cn(inputCls, "resize-y")} />
    </div>
    <div className="flex items-start gap-3 rounded-xl border border-[#cfe6fb] border-l-4 border-l-blue bg-[#eef7ff] p-[15px_17px]">
      ...
      <div>
        <div className="text-[13.5px] font-semibold text-[#0b5c9c]">Nghĩa vụ của bên khai thác</div>
        <div className="mt-0.5 text-[13px] leading-[1.55] text-[#2a6699]">
          Theo Nghị định <b>356/2025/NĐ-CP</b>, mọi khai thác hình ảnh, giọng nói, danh tính người
          khác phải có sự đồng ý minh thị và giấy phép hợp lệ. Thông tin pháp nhân sẽ được đối chiếu
          trước khi cấp phép.
        </div>
      </div>
    </div>
...
const ConfigStep2 = ({ usage, toggleUsage, elements, toggleElem, cfg, setCfg, d, termLabel }) => (
  <div data-reveal className="flex flex-col gap-[18px]">
    <div>
      <div className="mb-2.5 text-[13px] font-semibold text-ink">
        Loại hình khai thác <span className="font-medium text-mute">· chọn nhiều — khớp Phụ lục C</span>
      </div>
        ...
        {usageOptions.map((u) => (
          <button
            ...
            className={cn(
              ...
              usage[u.key] ? "border-blue bg-[#f2f8ff]" : "border-line bg-white"
            )}
          >
            <span className="flex-1">
              <span className="block text-[14.5px] font-semibold text-navy">{u.title}</span>
    ...
    <div>
      <div className="mb-1 text-[13px] font-semibold text-ink">
        Yếu tố nhân dạng <span className="font-medium text-mute">· chỉ hiện yếu tố chủ thể cho phép</span>
      </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  on ? "border-blue bg-[#eef7ff] text-[#0b5c9c]" : "border-line bg-white text-mute"
                )}
              >
    ...
    <div className="grid grid-cols-1 gap-3 min-[560px]:grid-cols-2">
      <div>
        <label className="mb-1.5 block text-[13px] font-semibold text-ink">Kênh phát hành</label>
        <select value={cfg.channel} onChange={(e) => setCfg((s) => ({ ...s, channel: e.target.value }))} className={inputCls}>
      ...
      <div>
        <label className="mb-1.5 block text-[13px] font-semibold text-ink">Lãnh thổ</label>
        <select value={cfg.territory} onChange={(e) => setCfg((s) => ({ ...s, territory: e.target.value }))} className={inputCls}>
    ...
    <div>
      ...
      <input
        type="range" min="3" max="36" step="3" value={cfg.term}
        onChange={(e) => setCfg((s) => ({ ...s, term: parseInt(e.target.value, 10) }))}
    ...
    <div>
      <div className="mb-2 text-[13px] font-semibold text-ink">
        Điều cấm bắt buộc của chủ thể <span className="font-medium text-mute">· khoá, không thể bỏ</span>
      </div>
      <div className="overflow-hidden rounded-xl border border-[#ffd0d2]">
        {d.prohibited.map((p, i) => (
          <div key={p} className={cn("flex items-center gap-[11px] bg-[#fff8f8] p-[12px_15px]", i > 0 && "border-t border-[#ffe0e1]")}>
            <IconLock size={15} stroke={1.7} className="shrink-0 text-[#9f0712]" />
            <span className="flex-1 text-[13.5px] font-medium text-[#7a2226]">{p}</span>
            <span className="rounded-full bg-[#ffe2e2] px-2.5 py-[3px] text-[11px] font-bold text-[#9f0712]">Khoá</span>
          </div>
        ))}
      </div>
    </div>
...
const ConfigStep3 = ({ priceModel, setPriceModel, cfg, setCfg, costLines, totalLabel, totalValue }) => {
  const models = [
    { key: "fixed", title: "Giá cố định", desc: "Trả trọn gói theo phạm vi & thời hạn." },
    { key: "share", title: "Chia sẻ doanh thu (%)", desc: "Chi trả theo tỷ lệ doanh thu khai thác, đối soát định kỳ." },
    { key: "propose", title: "Đề xuất giá của bạn", desc: "Gửi mức giá thương lượng để chủ thể duyệt." },
  ];
  return (
    <div data-reveal className="flex flex-col gap-4">
      <div>
        <div className="mb-2.5 text-[13px] font-semibold text-ink">Cơ chế tài chính</div>
        <div className="flex flex-col gap-2.5">
          {models.map((m) => (
            <button
              key={m.key}
              ...
              className={cn(
                ...
                priceModel === m.key ? "border-blue bg-[#f2f8ff]" : "border-line bg-white"
              )}
            >
              <span className="flex-1">
                <span className="block text-[14.5px] font-semibold text-navy">{m.title}</span>
                <span className="mt-0.5 block text-[12.5px] text-mute">{m.desc}</span>
              </span>
      ...
      {priceModel === "propose" && (
        <div data-reveal>
          ...
          <input
            type="range" min="3" max="80" value={cfg.propose}
            onChange={(e) => setCfg((s) => ({ ...s, propose: parseInt(e.target.value, 10) }))}
            className="h-1.5 w-full accent-blue"
          />
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-[14px] border border-line">
        ...
        <div className="flex flex-col gap-2.5 p-[16px_18px]">
          {costLines.map((c) => (
            <div key={c.label} className="flex justify-between gap-3 text-[13.5px]">
          ...
          <div className="flex items-baseline justify-between gap-3 border-t border-dashed border-line pt-3">
            ...
            <span className="font-serif text-[26px] font-bold text-navy">{totalValue}</span>
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2.5 rounded-[10px] border border-line bg-ivory p-[11px_14px] text-xs text-mute">
        ...
        Bao gồm phí bảo chứng &amp; thu hộ của Hiệp hội. Thanh toán chỉ thực hiện sau khi hai bên ký.
      </div>
    </div>
  );
};
...
const ConfigStep4 = ({ comp, setComp }) => {
  const items = [
    { key: "provenance", node: <>Tôi cam kết <b>gắn nhãn provenance</b> lên mọi nội dung tạo ra từ nhân dạng được cấp phép (nguồn gốc + mã giấy phép có thể kiểm chứng).</> },
    { key: "scope", node: <>Tôi <b>không sử dụng ngoài phạm vi</b> đã cấu hình (loại hình, yếu tố, kênh, lãnh thổ, thời hạn).</> },
    { key: "dlcn", node: <>Tôi tuân thủ pháp luật <b>Bảo vệ dữ liệu cá nhân</b> (Luật 91/2025/QH15) trong toàn bộ quá trình khai thác.</> },
    { key: "takedown", node: <>Tôi chấp nhận <b>gỡ nhanh</b> nội dung khi có tranh chấp hoặc chủ thể rút uỷ quyền.</> },
  ];
  return (
    <div data-reveal className="flex flex-col gap-[13px]">
      {items.map((it) => (
        ...
        >
          <input
            type="checkbox"
            checked={comp[it.key]}
            onChange={(e) => setComp((s) => ({ ...s, [it.key]: e.target.checked }))}
            className="mt-0.5 h-[18px] w-[18px] shrink-0 accent-blue"
          />
          <span className="text-[13.5px] leading-[1.55] text-ink">{it.node}</span>
        </label>
      ))}
      <div
      ...
      >
        <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[11px] bg-[rgba(199,154,43,.16)]">
          <DuotoneIcon name="fingerprint-1" size={22} color="#e0b64a" secondaryColor="#ecd9a3" />
        </span>
        <div>
          <div className="text-sm font-bold text-white">Provenance &amp; Watermark API</div>
          <div className="mt-[3px] text-[13px] leading-[1.55] text-[#c3cede]">
            Tích hợp API để tự động chèn watermark bất khả kiến &amp; ghi nguồn gốc vào từng nội dung —
            bằng chứng khai thác hợp pháp cho toàn bộ vòng đời giấy phép.
          </div>
          <a href="#" className="mt-2 inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-gold-2">
...
const ConfigStep5 = ({ contractBlocks, signed, setSigned, payMethod, setPayMethod }) => (
  <div data-reveal className="flex flex-col gap-4">
    <div className="overflow-hidden rounded-[13px] border border-line">
      ...
      <div className="grid grid-cols-1 min-[560px]:grid-cols-2">
        {contractBlocks.map((b) => (
          <div key={b.n} className="flex items-start gap-2.5 border-t border-line p-[12px_16px]">
            ...
            <div>
              <div className="text-[13px] font-semibold text-navy">{b.title}</div>
    ...
    >
      <span className="inline-flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[11px] bg-[rgba(12,140,233,.1)]">
        {signed ? <IconCheck size={22} stroke={1.8} style={{ color: OK }} /> : <IconWritingSign size={22} stroke={1.6} className="text-blue" />}
      </span>
      <span className="flex-1">
        <span className="block text-[15px] font-semibold text-navy">
          {signed ? "Đã ký số hợp đồng" : "Ký hợp đồng điện tử"}
        </span>
        <span className="mt-0.5 block text-[13px] text-mute">Ký số bằng chữ ký điện tử của pháp nhân.</span>
      </span>
      <span className="shrink-0 text-[13px] font-semibold text-blue-2">{signed ? "Ký lại" : "Ký ngay"}</span>
    </button>

    <div>
      <div className="mb-2 text-[13px] font-semibold text-ink">Phương thức thanh toán</div>
      <div className="grid grid-cols-1 gap-2.5 min-[560px]:grid-cols-2">
        {[
          ...
          <button
            ...
            className={cn(
              "w-full rounded-[13px] border-[1.5px] p-[13px_15px] text-left transition",
              payMethod === key ? "border-blue bg-[#f2f8ff]" : "border-line bg-white"
            )}
          >
    ...
    <div className="flex items-start gap-3 rounded-xl border border-[#f5e2b8] border-l-4 border-l-gold bg-[#fff8ec] p-[14px_16px]">
      ...
      <div className="text-[13px] leading-[1.55] text-[#7a5a10]">
        Hệ thống <b>chỉ khởi tạo yêu cầu thanh toán</b> — không tự động chuyển tiền. Dòng tiền được
        giải ngân qua tài khoản tách bạch sau khi giấy phép ghi nhận thành công.
      </div>
    </div>

    <div className="flex items-center gap-2.5 rounded-[10px] border border-line bg-ivory p-[11px_14px] text-xs text-mute">
      <IconInfoCircle size={15} stroke={1.6} className="shrink-0 text-blue" />
      Khi hoàn tất, hệ thống ghi <b className="font-semibold text-navy">tem thời gian</b> &amp;{" "}
      <b className="font-semibold text-navy">mã băm SHA-256</b> của hợp đồng vào nhật ký bất biến.
    </div>
  </div>
);
...
const LicenseIssued = ({ d, licenseCode, operatorName, usageSummary, elementSummary, scopeSummary, priceModelLabel, totalValue, expiryDate, onBackToMarket }) => (
  <section
    data-reveal
    className="relative overflow-hidden text-[#eaf0f8]"
  ...
  >
    ...
    <div className="relative mx-auto max-w-[820px] px-8 pb-[30px] pt-16 text-center">
      ...
      <p className="mx-auto mt-3.5 max-w-[560px] text-base leading-[1.65] text-[#c3cede]">
        Giấy phép có hiệu lực trong phạm vi đã cấu hình, kèm bằng chứng bất biến. Tra cứu công khai bất
        cứ lúc nào.
      </p>
    </div>

    <div className="relative mx-auto max-w-[820px] px-8 pb-[30px]">
      <div className="overflow-hidden rounded-2xl border border-[#e3ddca] shadow-[0_30px_70px_rgba(0,0,0,.4)]" style={{ background: "linear-gradient(180deg,#fdfbf5,#f7f2e6)" }}>
        <div className="flex items-start justify-between gap-4 border-b border-dashed border-[#d8cfb4] p-[24px_28px]">
          <div>
            ...
            <div className="mt-[5px] font-serif text-[24px] font-bold tracking-[0.04em] text-navy">{licenseCode}</div>
            <div className="mt-1 text-[12.5px] text-[#8a7a4e]">
              Cấp cho <b className="text-navy">{operatorName}</b> · khai thác nhân dạng của{" "}
        ...
        <div className="grid grid-cols-1 gap-x-7 gap-y-[18px] p-[22px_28px] min-[560px]:grid-cols-2">
          {[
            ...
            <div key={k}>
              <div className="text-[11px] font-semibold uppercase tracking-[0.04em] text-[#8a93a3]">{k}</div>
              <div className="mt-[3px] text-[14.5px] font-medium leading-[1.5] text-navy">{v}</div>
            </div>
          ))}
          <div className="col-span-full flex flex-wrap items-center gap-2.5 border-t border-dashed border-[#d8cfb4] pt-4">
            <span className="inline-flex items-center gap-2 rounded-full border border-[#bcdffb] bg-[rgba(12,140,233,.1)] px-[13px] py-[7px] text-[12.5px] font-semibold text-[#0b5c9c]">
              <DuotoneIcon name="fingerprint-1" size={15} color="#0c8ce9" secondaryColor="#bcdffb" />
        ...
        <div className="flex items-center gap-2.5 border-t border-dashed border-[#d8cfb4] bg-[rgba(25,40,61,.04)] p-[16px_28px] text-[12.5px] text-[#5b6b82]">
          ...
          Ghi nhận trong sổ đăng bạ của Hiệp hội bảo chứng nhân dạng số · bằng chứng bất biến.
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        ...
        <a href="/#tracuu" className="inline-flex items-center gap-2 rounded-xl border border-white/[.24] px-6 py-3.5 text-[15px] font-semibold text-[#eaf0f8]">
    ...
    <div className="bg-ivory px-8 pb-[60px] pt-12">
      <div className="mx-auto max-w-[900px]">
        <div className="mb-[26px] text-center">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-gold">Nghĩa vụ sau cấp phép</div>
          <h2 className="mt-2 font-serif text-[24px] font-bold text-navy">Giữ giấy phép luôn hợp lệ</h2>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {postLicenseDuties.map((duty) => (
            <div key={duty.title} className="flex flex-col gap-3 rounded-[14px] border border-line bg-white p-[22px] shadow-[0_6px_18px_rgba(15,23,43,.05)]">
              <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: duty.iconBg }}>
                <DuotoneIcon name={duty.icon} size={24} color={duty.color} secondaryColor={duty.secondary} />
              </span>
              <div className="text-[15.5px] font-semibold text-navy">{duty.title}</div>
              <div className="text-[13.5px] leading-[1.55] text-mute">{duty.desc}</div>
            </div>
          ))}
        </div>
